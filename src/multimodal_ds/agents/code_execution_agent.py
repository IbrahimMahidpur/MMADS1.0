...
class CodeExecutionAgent:
    ...
    def execute_task(self, task: dict, data_context: str = "", file_paths: Optional[list] = None, max_retries: int = 2) -> dict:
        ...
        with agent_span(self.AGENT_NAME, self.session_id, self._tracker) as span:
            ...
            if not code:
                ...
                code = self._generate_code(simplified_desc, data_context[:500], "")
            if not code:
    ...
    def _generate_code(self, task_desc: str, data_context: str, past_context: str) -> str:
        ...
        try:
            ...
            # Extract code from response (handle ```python fences)
    ...
    def _extract_code(self, text: str) -> str:
        ...
        if m:
            code = m.group(1).strip()
            # Sanity check: fenced content must look like Python, not prose
            if code and any(
                kw in code
                for kw in ('import ', 'def ', 'class ', 'print(', '=', 'pd.', 'df')
            ):
                return code

        # Last resort: if the entire response looks like Python (no fence),
        # return it directly. Guard: must start with import/def/# or have
        # assignment — never return pure prose.
        stripped = text.strip()
        ...
        # Could not extract valid Python — return empty so caller can handle
