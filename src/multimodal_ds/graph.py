...
def _executor_node(state):
    ...
    return {
        ...
        "step_file_map":     {**state.get("step_file_map", {}), step_idx: files},
    }
...
def _reviewer_node(state):
    ...
    for i, task in enumerate(tasks):
        ...
        step_files  = step_file_map.get(str(i), [])
    ...
    if not state.get("_last_success", True):
        ...
        if code_outputs:
            ...
            if isinstance(last_output, str):
                ...
                # Only trip the gate on real Python execution failures,
                # not on code-generation failures (those are handled by
                # the executor's own retry logic before returning).
                if "traceback (most recent call last)" in lowered:
                    gate_passed = False
                    gate_reasons.append("Execution returned non-zero exit code")
                elif "code generation failed" in lowered:
                    # Code generation failure — don't retry via reflection,
                    # it won't help. Let it through so executor advances
                    # to the next task instead of looping.
                    logger.warning("[Gate] Code generation failed — advancing to next task instead of retrying")
                    # Mark as passed so we don't infinite-loop, but record the skip
                    gate_passed = True
